...
let lastTouchEnd = 0; let lastTouchTarget = null;
document.addEventListener("touchend", (event) => {
  const now = Date.now();
  if (event.target === lastTouchTarget && now - lastTouchEnd <= 500) {
    event.preventDefault(); lastTouchEnd = 0; lastTouchTarget = null; return;
  }
  lastTouchEnd = now; lastTouchTarget = event.target;
}, { passive: false });
